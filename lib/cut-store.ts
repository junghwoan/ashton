...
const DB_NAME = "djty-cut";
const CHANNEL = "djty-cut-draft";
...
export async function readStoredDraft(): Promise<CutDraft | null> {
  const db = await openDb();
  const stored = await requestToPromise(db.transaction("draft").objectStore("draft").get("current"));
  db.close();
  return isCutDraft(stored) ? stored : null;
}

export async function writeStoredDraft(draft: CutDraft) {
...
export async function publishedDraft(): Promise<CutDraft | null> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/cut-copy.json`, { cache: "no-store" });
    if (!response.ok) return null;
    const data: unknown = await response.json();
    return isCutDraft(data) ? data : null;
  } catch {
    return null;
  }
}

export async function loadCutDraft(): Promise<CutDraft> {
  const stored = await readStoredDraft();
  if (stored) return stored;
  const published = await publishedDraft();
  return published ?? defaultCutDraft();
}

export async function resolveDraftImages(draft: CutDraft) {
...
export async function writeDraftToProject(draft: CutDraft) {
  let dir: DirHandle;
  try {
  ...
  const next: CutDraft = structuredClone(draft);
  const db = await openDb();
  for (const src of idbSources(next)) {
    ...
    const path = `/media/edits/${id}.jpg`;
    if (next.green.src === src) next.green = { ...next.green, src: path };
    next.releases = next.releases.map((release) => (release.cover === src ? { ...release, cover: path } : release));
    next.room = next.room.map((item) => (item.src === src ? { ...item, src: path } : item));
    next.elsewhere = next.elsewhere.map((item) => (item.src === src ? { ...item, src: path } : item));
  }
  db.close();
  next.updatedAt = Date.now();
  await fileIn(dir, "public/cut-copy.json", JSON.stringify(next, null, 2));
